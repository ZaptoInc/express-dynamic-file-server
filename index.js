...
app.all("*", function (req, res) {
  try {
    ...
    var folderConfig = getFolderConfig(
      ...
      req.hostname,
      currentPath
    ); //get the folder config for this request
    ...
    try {
      ...
      if (folderConfig.config.allowedHostname.length > 0) {
        if (
          doesMatchFromList(folderConfig.config.allowedHostname, req.hostname)
        ) {
        ...
        if (pathIsFolder) {
          ...
          if (folderConfig.config.onlineFolder) {
            ...
            childs.forEach((child) => {
              if (
                !child.name.endsWith(".fsconfig") &&
                !doesMatchFromList(
                  folderConfig.config.disallowedFiles,
                  child.name
                )
              ) {
            ...
            res.render(onlineFolderRenderer, {
              ...rendererParams,
              req,
              res,
              childs: formatedChilds,
              config: folderConfig,
            });
          } else {
            sendFile = false;
          ...
          if (
            (!fileStat || fileStat.isDirectory()) &&
            fs.existsSync(filePath + ".ejs")
          ) {
          ...
          if (fileStat && !fileStat.isDirectory()) {
            ...
            if (
              doesMatchFromList(folderConfig.config.disallowedFiles, filePath)
            ) {
              sendFile = false;
            }
    ...
    executeError(
      500,
      "errors/500.ejs",
      { config: defaultPath, URL: decodeURI(req.originalUrl.split("?")[0]) },
      req,
      res,
      error
    );
...
const getFolderConfig = function (
  path,
  workingPath,
  domain = "127.0.0.1",
  URL = "/",
  currentConfig = { ...defaultPath },
  relativePath = null
) {
  ...
  path.split("/").forEach((pathPart) => {
    ...
    var currentFsConfig = getFsConfig(
      path_.join(__dirname, workingPath, pathPart) + ".fsconfig"
    );
    //if the fs config does not exists and the workingPath does not ends by '/'
    if (!currentFsConfig && !workingPath.endsWith("/")) {
      //add '/' at the end of the workingPath
      workingPath = workingPath + "/";
      //retries to get the fs config of the path
      currentFsConfig = getFsConfig(
        path_.join(__dirname, workingPath, pathPart) + ".fsconfig"
      );
    }
    ...
    if (currentFsConfig) {
      ...
      if (currentFsConfig.folder && currentFsConfig.folder !== workingPath) {
      ...
      if (
        (currentFsConfig.pathRewrite && currentFsConfig.pathRewrite[domain]) ||
        (currentFsConfig.folder && currentFsConfig.folder !== workingPath)
      ) {
        //get the config of the folder after rerouting the request
        var newConfig = getFolderConfig(
          leftPath,
          workingPath,
          domain,
          URL,
